...
var (
	accessSecret  = []byte("access_select_example_change_me")
	refreshSecret = []byte("refresh_select_example_change_me")
...
func main() {
	fmt.Println("== JWT single-file demo ==")

	access, refresh, err := GenerateTokens(12345, "admin")
	if err != nil {
		panic(err)
	}
	...
	if ac, err := VerifyAccessToken("Bearer " + access); err != nil {
		fmt.Println("Access verify error:", err)
	} else {
		fmt.Printf("Access ok. uid=%d role=%s exp=%s\n", ac.UserID, ac.Role, ac.ExpiresAt.Time.Format(time.RFC3339))
	}

	fmt.Println("\n-- Verify Refresh Token --")

	if rc, err := VerifyRefreshToken(refresh); err != nil {
		fmt.Println("Refresh verify error:", err)
	} else {
		fmt.Printf("Refresh OK. uid=%d role=%s exp=%s\n", rc.UserID, rc.Role,
			rc.ExpiresAt.Time.Format(time.RFC3339))
	}
	fmt.Println("\nDone.")
}
